...
when not defined(js) and not defined(nodejs):
  type
    ...
    ClassList* = ref object of RootObj
    ...
    ValidityState* = ref object ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/ValidityState>`_
      badInput*: bool
      customError*: bool
    ...
    Blob* = ref object of RootObj ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/Blob>`_
      size*: int
      `type`*: cstring

    File* = ref object of Blob ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/File>`_
      lastModified*: int
      name*: cstring

    TextAreaElement* = ref object of Element ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/HTMLTextAreaElement>`_
      value*: cstring
      selectionStart*, selectionEnd*: int
      selectionDirection*: cstring
      rows*, cols*: int

    InputElement* = ref object of Element ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement>`_
    ...
    FormElement* = ref object of Element ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/HTMLFormElement>`_
    ...
    Event* = ref object of RootObj ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/Event>`_
      bubbles*: bool
      cancelBubble*: bool
      cancelable*: bool
      composed*: bool
    ...
    UIEvent* = ref object of Event ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/UIEvent>`_
      detail*: int64
      view*: Window

    KeyboardEvent* = ref object of UIEvent ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent>`_
    ...
    MouseEvent* = ref object of UIEvent ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent>`_
      altKey*, ctrlKey*, metaKey*, shiftKey*: bool
      button*: int
      buttons*: int
      clientX*, clientY*: int
    ...
    DataTransferItem* = ref object of RootObj ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/DataTransferItem>`_
      kind*: cstring
      `type`*: cstring

    DataTransfer* = ref object of RootObj ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/DataTransfer>`_
      dropEffect*: cstring
    ...
    DragEvent* = object of MouseEvent
      ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/DragEvent>`_
      dataTransfer*: DataTransfer

    ClipboardEvent* = object of Event
      ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/ClipboardEvent>`_
      clipboardData*: DataTransfer

    StorageEvent* = ref object of Event ## see `docs<https://developer.mozilla.org/en-US/docs/Web/API/StorageEvent>`_
      key*: cstring
      newValue*, oldValue*: cstring
      storageArea*: Storage
      url*: cstring

    TouchList* = ref object of RootObj
      length*: int

    Touch* = ref object of RootObj
      identifier*: int
    ...
    History* = ref object of RootObj
      length*: int

    Navigator* = ref object of RootObj
      appCodeName*: cstring
      appName*: cstring
      appVersion*: cstring
    ...
    Plugin* = object of RootObj
      description*: cstring
      filename*: cstring
      name*: cstring

    MimeType* = object of RootObj
      description*: cstring
      enabledPlugin*: ref Plugin
      suffixes*: seq[cstring]
      `type`*: cstring

    LocationBar* = object of RootObj
      visible*: bool
    MenuBar* = LocationBar
    ...
    TimeOut* = ref object of RootObj
    Interval* = ref object of RootObj
    ...
    MediaQueryList* = ref object of EventTarget
      matches*: bool
      media*: cstring
  ...
  since (1, 3):
    type
      ...
      FileReader* = ref object of EventTarget
      ...
      RootNodeOptions* = object of RootObj
        composed*: bool
      DocumentOrShadowRoot* = object of RootObj
        activeElement*: Element
        # styleSheets*: StyleSheetList
      ShadowRoot* = ref object of DocumentOrShadowRoot
        delegatesFocus*: bool
        host*: Element
        innerHTML*: cstring
      ...
      HTMLSlotElement* = ref object of RootObj
        name*: cstring
...
when defined(nodejs):
  ...
  when defined(js):
    ...
    # proc getBoundingClientRect*(e: Node): BoundingRect = discard

    # proc querySelector*(n: Node, selectors: cstring): Element = discard
    # proc querySelectorAll*(n: Node, selectors: cstring): seq[Element] = discard
    # proc querySelector*(d: Document, selectors: cstring): Element = discard
    # proc querySelectorAll*(d: Document, selectors: cstring): seq[Element] = discard
    # # Event "methods"
    # proc preventDefault*(ev: Event) = discard
    # proc stopImmediatePropagation*(ev: Event) = discard
    # proc stopPropagation*(ev: Event) = discard

    # proc getAttribute*(n: Node, attr: cstring): cstring = discard
    # proc getAttributeNode*(n: Node, attr: cstring): Node = discard
    # proc hasAttribute*(n: Node, attr: cstring): bool = discard

    # since (1, 5):
    #   proc elementsFromPoint*(n: DocumentOrShadowRoot; x, y: float): seq[Element] = discard
    ...
    proc appendChild*(n, child: Node) = discard
    proc removeChild*(n, child: Node) = discard
    proc remove*(child: Node) = discard
    proc replaceChild*(n, newNode, oldNode: Node) = discard
    proc insertBefore*(n, newNode, before: Node) = discard
    proc getElementById*(d: Document, id: cstring): Element = discard
    proc createElement*(d: Document, identifier: cstring): Element = discard
    proc createElementNS*(d: Document, namespaceURI, qualifiedIdentifier: cstring): Element = discard
    proc createTextNode*(d: Document, identifier: cstring): Node = discard
    proc createComment*(d: Document, data: cstring): Node = discard

    proc setTimeout*(action: proc(); ms: int): TimeOut = discard
    proc clearTimeout*(t: TimeOut) = discard
    proc setInterval*(action: proc(); ms: int): Interval = discard
    proc clearInterval*(i: Interval) = discard

    # EventTarget "methods"
    proc addEventListener*(et: EventTarget, ev: cstring, cb: proc(ev: Event), useCapture: bool = false) = discard
    proc addEventListener*(et: EventTarget, ev: cstring, cb: proc(ev: Event), options: AddEventListenerOptions) = discard
    proc dispatchEvent*(et: EventTarget, ev: Event) = discard
    proc removeEventListener*(et: EventTarget, ev: cstring, cb: proc(ev: Event), useCapture: bool = false) = discard
    proc removeEventListener*(et: EventTarget, ev: cstring, cb: proc(ev: Event), options: AddEventListenerOptions) = discard
    ...
    proc clearTimeout*(w: Window, timeout: TimeOut) = discard
    proc close*(w: Window) = discard
    proc confirm*(w: Window, msg: cstring): bool = discard
    proc disableExternalCapture*(w: Window) = discard
    proc enableExternalCapture*(w: Window) = discard
    proc find*(w: Window, text: cstring, caseSensitive = false,
              backwards = false): bool = discard
    proc focus*(w: Window) = discard
    proc forward*(w: Window) = discard
    proc getComputedStyle*(w: Window, e: Node, pe: Node = nil): Style = discard
      ## .. warning:: The returned Style may or may not be read-only at run-time in the browser. getComputedStyle is performance costly.

    proc handleEvent*(w: Window, e: Event) = discard
    proc home*(w: Window) = discard
    proc moveBy*(w: Window, x, y: int) = discard
    proc moveTo*(w: Window, x, y: int) = discard
    proc open*(w: Window, uri, windowname: cstring,
              properties: cstring = nil): Window = discard
    proc print*(w: Window) = discard
    proc prompt*(w: Window, text, default: cstring): cstring = discard
    proc resizeBy*(w: Window, x, y: int) = discard
    proc resizeTo*(w: Window, x, y: int) = discard
    proc routeEvent*(w: Window, event: Event) = discard
    proc scrollBy*(w: Window, x, y: int) = discard
    proc scrollTo*(w: Window, x, y: int) = discard
    proc setInterval*(w: Window, code: cstring, pause: int): Interval = discard
    proc setInterval*(w: Window, function: proc (), pause: int): Interval = discard
    proc setTimeout*(w: Window, code: cstring, pause: int): TimeOut = discard
    proc setTimeout*(w: Window, function: proc (), pause: int): Interval = discard
    proc stop*(w: Window) = discard
    proc requestAnimationFrame*(w: Window, function: proc (time: float)): int = discard
    proc cancelAnimationFrame*(w: Window, id: int) = discard
    proc matchMedia*(w: Window, mediaQueryString: cstring): MediaQueryList = discard

    # Node "methods"
    proc appendData*(n: Node, data: cstring) = discard
    proc cloneNode*(n: Node, copyContent: bool): Node = discard
    proc deleteData*(n: Node, start, len: int) = discard
    proc focus*(e: Node) = discard
    proc getAttribute*(n: Node, attr: cstring): cstring = discard
    proc getAttributeNode*(n: Node, attr: cstring): Node = discard
    proc hasAttribute*(n: Node, attr: cstring): bool = discard
    proc hasChildNodes*(n: Node): bool = discard
    proc normalize*(n: Node) = discard
    proc insertData*(n: Node, position: int, data: cstring) = discard
    proc removeAttribute*(n: Node, attr: cstring) = discard
    proc removeAttributeNode*(n, attr: Node) = discard
    proc replaceData*(n: Node, start, len: int, text: cstring) = discard
    proc scrollIntoView*(n: Node) = discard
    proc scrollIntoView*(n: Node, options: ScrollIntoViewOptions) = discard
    proc setAttribute*(n: Node, name, value: cstring) = discard
    proc setAttributeNode*(n: Node, attr: Node) = discard
    proc querySelector*(n: Node, selectors: cstring): Element = discard
    proc querySelectorAll*(n: Node, selectors: cstring): seq[Element] = discard
    proc compareDocumentPosition*(n: Node, otherNode:Node): int = discard
    proc lookupPrefix*(n: Node): cstring = discard
    proc lookupNamespaceURI*(n: Node): cstring = discard
    proc isDefaultNamespace*(n: Node): bool = discard
    proc contains*(n: Node): bool = discard
    proc isEqualNode*(n: Node): bool = discard
    proc isSameNode*(n: Node): bool = discard

    since (1, 3):
      proc getRootNode*(n: Node,options: RootNodeOptions): Node = discard

      # DocumentOrShadowRoot
      proc getSelection*(n: DocumentOrShadowRoot): Selection = discard
      proc elementFromPoint*(n: DocumentOrShadowRoot; x, y: float): Element = discard

      # shadow dom
      proc attachShadow*(n: Element): ShadowRoot = discard
      proc assignedNodes*(n: HTMLSlotElement; options: SlotOptions): seq[Node] = discard
      proc assignedElements*(n: HTMLSlotElement; options: SlotOptions): seq[Element] = discard

    # Document "methods"
    proc createAttribute*(d: Document, identifier: cstring): Node = discard
    proc getElementsByName*(d: Document, name: cstring): seq[Element] = discard
    proc getElementsByTagName*(d: Document, name: cstring): seq[Element] = discard
    proc getElementsByClassName*(d: Document, name: cstring): seq[Element] = discard
    proc insertNode*(range: Range, node: Node) = discard
    proc getSelection*(d: Document): Selection = discard
    proc handleEvent*(d: Document, event: Event) = discard
    proc open*(d: Document) = discard
    proc routeEvent*(d: Document, event: Event) = discard
    proc write*(d: Document, text: cstring) = discard
    proc writeln*(d: Document, text: cstring) = discard
    proc querySelector*(d: Document, selectors: cstring): Element = discard
    proc querySelectorAll*(d: Document, selectors: cstring): seq[Element] = discard

    # Element "methods"
    proc blur*(e: Element) = discard
    proc click*(e: Element) = discard
    proc focus*(e: Element) = discard
    proc handleEvent*(e: Element, event: Event) = discard
    proc select*(e: Element) = discard
    proc getElementsByTagName*(e: Element, name: cstring): seq[Element] = discard
    proc getElementsByClassName*(e: Element, name: cstring): seq[Element] = discard

    # FormElement "methods"
    proc reset*(f: FormElement) = discard
    proc submit*(f: FormElement) = discard
    proc checkValidity*(e: FormElement): bool = discard
    proc reportValidity*(e: FormElement): bool = discard

    # EmbedElement "methods"
    proc play*(e: EmbedElement) = discard
    proc stop*(e: EmbedElement) = discard

    # Location "methods"
    proc reload*(loc: Location) = discard
    proc replace*(loc: Location, s: cstring) = discard

    # History "methods"
    proc back*(h: History) = discard
    proc forward*(h: History) = discard
    proc go*(h: History, pagesToJump: int) = discard
    proc pushState*[T](h: History, stateObject: T, title, url: cstring) = discard

    # Navigator "methods"
    proc javaEnabled*(h: Navigator): bool = discard
    since (1, 3):
      proc canShare*(self: Navigator; data: cstring): bool  = discard          ## https://developer.mozilla.org/en-US/docs/Web/API/Navigator/canShare
      proc sendBeacon*(self: Navigator; url, data: cstring): bool = discard    ## https://developer.mozilla.org/en-US/docs/Web/API/Navigator/sendBeacon
      proc vibrate*(self: Navigator; pattern: cint): bool = discard            ## https://developer.mozilla.org/en-US/docs/Web/API/Navigator/vibrate
      proc vibrate*(self: Navigator; pattern: openArray[cint]): bool = discard ## https://developer.mozilla.org/en-US/docs/Web/API/Navigator/vibrate
      proc registerProtocolHandler*(self: Navigator; scheme, url, title: cstring) = discard ## https://developer.mozilla.org/en-US/docs/Web/API/Navigator/registerProtocolHandler

    # ClassList "methods"
    proc add*(c: ClassList, class: cstring) = discard
    proc remove*(c: ClassList, class: cstring) = discard
    proc contains*(c: ClassList, class: cstring): bool = discard
    proc toggle*(c: ClassList, class: cstring) = discard

    # Style "methods"
    proc getPropertyValue*(s: Style, property: cstring): cstring = discard
    proc removeProperty*(s: Style, property: cstring) = discard
    proc setProperty*(s: Style, property, value: cstring, priority = "") = discard
    proc getPropertyPriority*(s: Style, property: cstring): cstring = discard
    ...
    # KeyboardEvent "methods"
    proc getModifierState*(ev: KeyboardEvent, keyArg: cstring): bool = discard

    # MouseEvent "methods"
    proc getModifierState*(ev: MouseEvent, keyArg: cstring): bool = discard

    # TouchEvent "methods"
    proc identifiedTouch*(list: TouchList): Touch = discard
    proc item*(list: TouchList, i: int): Touch = discard

    # DataTransfer "methods"
    proc clearData*(dt: DataTransfer, format: cstring) = discard
    proc getData*(dt: DataTransfer, format: cstring): cstring = discard
    proc setData*(dt: DataTransfer, format: cstring, data: cstring) = discard
    proc setDragImage*(dt: DataTransfer, img: Element, xOffset: int, yOffset: int) = discard

    # DataTransferItem "methods"
    proc getAsFile*(dti: DataTransferItem): File = discard

    # InputElement "methods"
    proc setSelectionRange*(e: InputElement, selectionStart: int, selectionEnd: int, selectionDirection: cstring = "none") = discard
    proc setRangeText*(e: InputElement, replacement: cstring, startindex: int = 0, endindex: int = 0, selectionMode: cstring = "preserve") = discard
    proc setCustomValidity*(e: InputElement, error: cstring) = discard
    proc checkValidity*(e: InputElement): bool = discard

    # Blob "methods"
    proc slice*(e: Blob, startindex: int = 0, endindex: int = e.size, contentType: cstring = "") = discard

    # Performance "methods"
    proc now*(p: Performance): float = discard

    # Selection "methods"
    proc removeAllRanges*(s: Selection) = discard
    proc deleteFromDocument*(s: Selection) = discard
    proc getRangeAt*(s: Selection, index: int): Range = discard
    converter toString*(s: Selection): cstring = discard
    proc `$`*(s: Selection): string = discard

    # Storage "methods"
    proc getItem*(s: Storage, key: cstring): cstring = discard
    proc setItem*(s: Storage, key, value: cstring) = discard
    proc clear*(s: Storage) = discard
    proc removeItem*(s: Storage, key: cstring) = discard

    proc setAttr*(n: Node; key, val: cstring) = discard
